...
export default function EventHero({eventName,eventDescription, hostDepartment, eventDay,onRegisterClick }) {
  ...
  return (
    <div className="p-8 h-screen gap-4 md:gap-0 flex flex-col bg-transparent justify-center items-center text-black bg-[radial-gradient(#00000089,transparent_2px)] [background-size:20px_20px] ">
      ...
      <p className='fade-up text-center bg-[linear-gradient(to_bottom,_#4870B6,_#8AAAFE)] text-white rounded-full px-3 py-1 fustat-heading text-sm md:text-lg tracking-wide font-medium uppercase my-3'>{eventDay==3 ? "Technotsav Day 1 & 2" :  `Technotsav Day ${eventDay}`}</p>
      <p className='fade-up fustat-heading text-lg text-center md:text-xl '> {fullHostDepartment}</p>
      <p className='fade-up manrope-paragraph'>Presents</p>
      <h1 className="fade-up text-6xl md:text-8xl spacecrusaders-heading gradient text-center mb-10 w-full">
        {eventName.split(' ').map((word, index) => (
          <span key={index} className="block">{word}</span>
        ))}
      </h1>
      <div className='fade-up flex gap-2 flex-row items-center justify-cente</div>r'>
