...
export default function EventContacts({ contacts }) {
    ...
    return (
        <div className="w-[80vw] mx-auto px-4 py-5 md:py-10">
            <h2 className="text-xl md:text-3xl font-bold text-center text-gray-800 mb-3 fustat-heading">
                <span className='gradient text-3xl md:text-5xl fustat-heading'> Contact</span> Us
            </h2>

            <p className="text-center text-gray-600 mb-5">Click on a contact card to copy the phone number to your clipboard.</p>

            <div className="flex flex-wrap justify-center gap-6 py-3 px-4 md:px-16">
                {contacts.map((contact, index) => (
                    <div
                        key={index}
                        className="bg-white border border-gray-200 shadow-lg rounded-2xl p-3 w-60 hover:shadow-2xl hover:scale-105 transition duration-300 ease-in-out flex flex-col justify-center items-center cursor-pointer"
