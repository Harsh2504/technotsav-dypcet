        ...
        <section className="max-w-4xl mx-auto px-4 py-10">
          ...
          <div className="space-y-6 ">
            {rules.map((rule, index) => (
              <div
                key={index}
                className="border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 bg-white"
              >
                <h3 className="text-xl font-semibold text-indigo-600 mb-2 flex items-center gap-2 fustat-heading">
                  <span className="inline-block bg-indigo-100 text-indigo-700 rounded-full px-3 py-1 fustat-heading text-sm font-medium">
                    Rule {index + 1}
                  </span>
                  {rule.title}
                </h3>
